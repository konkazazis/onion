...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> loadActivities(DateTime selectedDate, String userID) async {
    ...
    try {
      var resWork = await _shiftsService.fetchShiftsByMonth(
          "${selectedDate.month}-${selectedDate.year}", userID);
      var resPersonal = await _personalSerivce.fetchPersonalByMonth(
          "${selectedDate.month}-${selectedDate.year}", userID);
      var resPhysical = await _physicalSerivce.fetchPhysicalByMonth(
          "${selectedDate.month}-${selectedDate.year}", userID);
      var resSocial = await _socialSerivce.fetchSocialByMonth(
          "${selectedDate.month}-${selectedDate.year}", userID);
      var resHouse = await _houseSerivce.fetchHouseByMonth(
          "${selectedDate.month}-${selectedDate.year}", userID);
  ...
  Future<void> loadDetails() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('There was an error with loading your profile details')),
      );
      print("Error fetching details: $e");
    }
  }

  List<Map<String, dynamic>> getActivitiesForDay(
      List<Map<String, dynamic>> monthlyActivities, DateTime day) {
  ...
  Future<void> deleteActivity(
      List<Map<String, dynamic>> monthlyActivities, String id) async {
    try {
    ...
    if (filteredWork.isNotEmpty) {
      tabs.add(
        TabData(
          index: 1,
          title: Tab(
              text: "Work (${filteredWork.length})", icon: Icon(Icons.work)),
          content: SingleChildScrollView(
            child: Column(
              children: List.generate(filteredWork.length, (index) {
                        ...
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (result == 'refresh') {
                              await loadActivities(_selectedDay, widget.userid);
                              setState(() {
                                filteredWork = getActivitiesForDay(
                                    monthlyWork, _selectedDay);
                              });
                            }
                          },
                        ),
    ...
    if (filteredPersonal.isNotEmpty) {
      tabs.add(
        TabData(
          ...
          content: SingleChildScrollView(
            child: Column(
              children: List.generate(filteredPersonal.length, (index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text(
                      [eventDate, if (notes.trim().isNotEmpty) notes]
                          .join('\n'),
                    ),
                  ),
                );
    ...
    if (filteredPhysical.isNotEmpty) {
      tabs.add(
        TabData(
          ...
          content: SingleChildScrollView(
            child: Column(
              children: List.generate(filteredPhysical.length, (index) {
                        ...
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (result == 'refresh') {
                              await loadActivities(_selectedDay, widget.userid);
                              setState(() {
                                filteredPhysical = getActivitiesForDay(
                                    monthlyWork, _selectedDay);
                              });
                            }
                          },
    ...
    if (filteredSocial.isNotEmpty) {
      tabs.add(
        TabData(
          ...
          content: SingleChildScrollView(
            child: Column(
              children: List.generate(filteredSocial.length, (index) {
                        ...
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (result == 'refresh') {
                              await loadActivities(_selectedDay, widget.userid);
                              setState(() {
                                filteredSocial = getActivitiesForDay(
                                    monthlyWork, _selectedDay);
                              });
                            }
                          },
    ...
    if (filteredHouse.isNotEmpty) {
      tabs.add(
        TabData(
          ...
          content: SingleChildScrollView(
            child: Column(
              children: List.generate(filteredHouse.length, (index) {
          ...
          child:
                  ...
                  CalendarWidget(
                    ...
                    onDaySelected: (selectedDay, focusedDay) {
                      setState(() {
                        ...
                        filteredWork =
                            getActivitiesForDay(monthlyWork, selectedDay);
                        filteredPersonal =
                            getActivitiesForDay(monthlyPersonal, selectedDay);
                        filteredPhysical =
                            getActivitiesForDay(monthlyPhysical, selectedDay);
                        filteredSocial =
                            getActivitiesForDay(monthlySocial, selectedDay);
                    ...
                    onPageChanged: (focusedDay) {
                      ...
                      setState(() {
                        ...
                        filteredWork =
                            getActivitiesForDay(monthlyWork, _selectedDay);
                        filteredPersonal =
                            getActivitiesForDay(monthlyPersonal, _selectedDay);
                        filteredPhysical =
                            getActivitiesForDay(monthlyPhysical, _selectedDay);
                        filteredSocial =
                            getActivitiesForDay(monthlySocial, _selectedDay);
                        filteredHouse =
                            getActivitiesForDay(monthlyHouse, _selectedDay);
                      });
            ...
            _isLoading
                ...
                : filteredWork.isEmpty &&
                        ...
                        filteredSocial.isEmpty &&
                        filteredHouse.isEmpty
                    ? const Center(
                        child: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: const [
